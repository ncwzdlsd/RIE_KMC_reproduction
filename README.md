...
每个当前纳米颗粒-溶液界面 Ce/O 位点都对应一个候选 KMC 超声事件。程序用 n-fold-way 将这些等速事件聚合：总倾向等于“每界面位点倾向 × 当前界面位点数”，事件被选中后再等概率确定中心。这与显式列出全部中心严格等价，不是给整个颗粒设置一个与表面积无关的恒定总事件率。补充材料未公开每界面位点倾向，因此该值仍需标定。

溶液本体采用充分混合的隐式表示。无超声组使用论文 Fig. S34 的 `-0.60 eV` Ce/O 巨正则化学势；超声组除界面腐蚀事件外，还使用一个待标定的正化学势增量，表示论文所述“小而粗糙颗粒腐蚀后提高共享溶液 Ce/O 浓度并加速大颗粒再沉积”。这是单个显式生长颗粒对多颗粒熟化浴的降阶表示，不增加新的反应类型。Ir 使用守恒的有限前驱体储库，并按论文描述从盒子边缘吸附到 M 位点，再沿整个溶液可达的空 M 位点网络扩散。Ir 只有到达载体或已有金属 Ir 附近才能还原，因此盒内迁移态是 `Ir(ion)`，不会直接形成液相金属团簇。

Ir 储库以补充材料 Table S9 的最终 RIE-Ir/CeOₓ ICP-OES 组成为负载目标：Ir 为 15.60 wt%，Ce 为 74.96 wt%，对应 `Ir/Ce = 0.15170`。标准 5 nm 初始载体含约 1632 个 Ce 原子，目标负载量因而约为 248 个 Ir。由于补充材料没有公开前驱体保留率，默认不再用任意的 10% 保留率把投料放大十倍；有限储库直接使用目标尺度，并通过更符合 5–60 min 成核时间窗的异相还原速率使其抵达载体。Ir 吸附消耗一个储库原子，Ir 离子脱附归还一个；吸附速率乘以储库剩余比例，耗尽后自动变为零。高级入口仍可用 `--ir-precursor-atoms` 覆盖总投料。

Ir 形貌采用论文五类反应内的“扩散后还原”机制：离子态 Ir 的初始扩散频率因子为 `2.0 s^-1`，异相还原频率因子为 `0.5 s^-1`。后者是依据“5 min 已出现 Ir、60 min 内成核基本完成”的时间窗给出的待标定初值，使到达载体的 Ir 能与逃逸竞争；它不是论文公开数值。O 位点的局部能量同时计入 Ce-O 和 Ir-O 邻接，因此 CeOₓ 能够在 Ir 周围继续生长。金属 Ir 表面扩散不在论文列出的五类反应中，当前没有擅自加入。
...
| 超声浴 Ce/O 化学势增量 | 未公开，需标定 | `sonication_chemical_potential_shift_ev` |
...
论文补充图 S34/S35 的单颗粒 KMC 会逐渐暴露稳定 (111) 面，最终本来就具有多面体/Wulff 特征；实验 TEM 和 Fig. S33 的“近圆”是有限分辨率投影下的粗糙颗粒外轮廓。程序因此不加入论文之外的强制球形约束，而是在 `metrics.csv` 中同时报告载体轴向尺寸比、形状各向异性和表面径向离散度，用来区分整体近球、各向拉长和过度规则化。超声浴富集与局部随机腐蚀共同维持较圆钝的粗糙外形。
...
补充材料没有公开完整动力学参数。准备阶段可运行一次 `preparation/calibrate_parameters.py`；程序默认执行 40 次搜索、每个候选参数运行 5 个随机重复，并用 Table S3 的相对粒径曲线和 Table S5 的 Ce 溶解曲线，拟合 Ce/O 交换时间尺度、每界面位点超声事件倾向和超声造成的共享浴化学势增量。
...
当前 `2.0 s^-1` 的单跳扩散频率与 `0.5 s^-1` 的载体接触还原频率，使 Ir 保留显式逐跳输运，同时在抵达载体后能在论文的 5–60 min 成核时间窗内冻结。该尺度不能替代作者未公开的定量拟合。
...
- `trajectory.xyz`：0、5、30、60、120、180 min 的有/无超声并排轨迹，只显示最大 CeOₓ 连通主晶体以及与该主晶体连接的 Ir；脱落的 Ce/O/Ir 碎片不再混入主颗粒图像；
- `snapshots/*.xyz`：每个取样时间点可单独打开的相同过滤结果；
- `metrics.csv`：相同物理时间下的两组指标及差值，包含 Ir 储库、守恒误差、团簇数、盒内未连接 Ir、负载/嵌入 Ir、`detached_support_atoms`，以及主晶体径向离散度、轴向尺寸比和形状各向异性；
...
`local_kmc.py` 分别维护 Ce/O、盒边缘 Ir 交换、Ir 氧化还原和 Ir 离子扩散速率桶。普通事件只刷新两层邻域；超声事件只刷新局部腐蚀区域。外部溶液连通性按 `--reconcile-every` 周期全局校正，避免封闭孔洞造成长期误差。每个条件的 Ce/O 化学势在单次运行中固定，超声组的常数增量表示共享熟化浴富集；各配位速率桶的速率因此仍可保持不变。Ir 交换桶保存满浓度下的本征速率；事件抽样时仅对吸附桶乘以实时储库余量比例，因此储库变化不需要全局扫描，同时严格保持 `溶液 Ir + 晶格 Ir = 初始投料`。
